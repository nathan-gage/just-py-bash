...
// dist/commands/js-exec/js-exec.js
import { AsyncLocalStorage } from "node:async_hooks";
import { randomBytes } from "node:crypto";
import { fileURLToPath } from "node:url";
import { Worker } from "node:worker_threads";
var DEFAULT_JS_TIMEOUT_MS = 1e4;
var DEFAULT_JS_NETWORK_TIMEOUT_MS = 6e4;
var jsExecAsyncContext = new AsyncLocalStorage();
var JS_EXEC_HELP = `js-exec - Sandboxed JavaScript/TypeScript runtime with Node.js-compatible APIs
...
var workerPath = fileURLToPath(new URL("./js-exec-worker.js", import.meta.url));
...
async function executeJSInner(jsCode, ctx, scriptPath, scriptArgs = [], bootstrapCode, isModule, stripTypes) {
  ...
  const wrappedExec = execFn ? (command, options) => jsExecAsyncContext.run(true, () => execFn(command, options)) : void 0;
  ...
  const timeoutMs = ctx.fetch ? Math.max(userTimeout, DEFAULT_JS_NETWORK_TIMEOUT_MS) : userTimeout;
