...
var lsCommand = {
  ...
  async execute(args, ctx) {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      if (directoryOnly) {
        const fullPath = ctx.fs.resolvePath(ctx.cwd, path2);
        try {
          const stat = await ctx.fs.stat(fullPath);
          if (longFormat) {
            const mode = stat.isDirectory ? "drwxr-xr-x" : "-rw-r--r--";
            const suffix = classifyFiles ? classifySuffix(await ctx.fs.lstat(fullPath)) : stat.isDirectory ? "/" : "";
            ...
            stdout += `${mode} 1 user user ${sizeStr} ${dateStr} ${path2}${suffix}
`;
          } else {
            const suffix = classifyFiles ? classifySuffix(await ctx.fs.lstat(fullPath)) : "";
            stdout += `${path2}${suffix}
`;
          }
        } catch {
          stderr += `ls: cannot access '${path2}': No such file or directory
`;
          exitCode = 2;
        }
        continue;
      ...
      if (path2.includes("*") || path2.includes("?") || path2.includes("[")) {
        const result = await listGlob(path2, ctx, showAll, showAlmostAll, longFormat, reverse, humanReadable, sortBySize, classifyFiles);
        stdout += result.stdout;
        stderr += result.stderr;
        if (result.exitCode !== 0)
          exitCode = result.exitCode;
      } else {
        const result = await listPath(path2, ctx, showAll, showAlmostAll, longFormat, recursive, paths.length > 1, reverse, humanReadable, sortBySize, classifyFiles);
        stdout += result.stdout;
        stderr += result.stderr;
        if (result.exitCode !== 0)
          exitCode = result.exitCode;
...
`,
  ...
  if (sortBySize) {
    ...
    for (const match2 of matches) {
      const fullPath = ctx.fs.resolvePath(ctx.cwd, match2);
      try {
        const stat = await ctx.fs.stat(fullPath);
        matchesWithSize.push({ path: match2, size: stat.size ?? 0 });
      } catch {
        matchesWithSize.push({ path: match2, size: 0 });
      }
    }
  ...
  if (longFormat) {
    ...
    for (const match2 of matches) {
      ...
      try {
        ...
        const mtime = stat.mtime ?? /* @__PURE__ */ new Date(0);
        const dateStr = formatDate(mtime);
        lines.push(`${mode} 1 user user ${sizeStr} ${dateStr} ${match2}${suffix}`);
      } catch {
        lines.push(`-rw-r--r-- 1 user user     0 Jan  1 00:00 ${match2}`);
      }
    }
  ...
  if (classifyFiles) {
    ...
    for (const match2 of matches) {
      ...
      try {
        ...
        classified.push(`${match2}${classifySuffix(stat)}`);
      } catch {
        classified.push(match2);
      }
    }
...
async function listPath(path2, ctx, showAll, showAlmostAll, longFormat, recursive, showHeader, reverse = false, humanReadable = false, sortBySize = false, classifyFiles = false, _isSubdir = false) {
  const showHidden = showAll || showAlmostAll;
  const fullPath = ctx.fs.resolvePath(ctx.cwd, path2);
  try {
    const stat = await ctx.fs.stat(fullPath);
    if (!stat.isDirectory) {
      const fileSuffix = classifyFiles ? classifySuffix(await ctx.fs.lstat(fullPath)) : "";
      if (longFormat) {
        const size = stat.size ?? 0;
        const sizeStr = humanReadable ? formatHumanSize(size).padStart(5) : String(size).padStart(5);
        const mtime = stat.mtime ?? /* @__PURE__ */ new Date(0);
        const dateStr = formatDate(mtime);
        return {
          stdout: `-rw-r--r-- 1 user user ${sizeStr} ${dateStr} ${path2}${fileSuffix}
`,
          stderr: "",
          exitCode: 0
        };
      }
      return { stdout: `${path2}${fileSuffix}
`, stderr: "", exitCode: 0 };
    }
      ...
      for (let i = 0; i < regularEntries.length; i += DEFAULT_BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(batch.map(async (entry) => {
          ...
          try {
            ...
            const suffix = classifyFiles ? classifySuffix(await ctx.fs.lstat(entryPath)) : entryStat.isDirectory ? "/" : "";
...
`
    ...
    if (recursive) {
      ...
      for (let i = 0; i < dirEntries.length; i += DEFAULT_BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(batch.map(async (dir) => {
          const subPath = path2 === "." ? `./${dir.name}` : `${path2}/${dir.name}`;
    ...
    return {
      stdout: "",
      stderr: `ls: ${path2}: No such file or directory
`,
      exitCode: 2
    };
  }
